...
| Create Escalation | Server-assigns `number`/`opened_at`; client supplies `account_id`, `summary`, optional `incident_number`/`owner` — mirrors `POST /incidents`' server-assigned-identifier pattern | must-have | v2 | specified |
