# Product Vision: mock-servicenow
...
No modules chartered yet — this repo has PRD.md but no code. Run `/adev:brainstorm` to charter
its module layout (e.g. `itsm-api`, `mcp-server`) following `mock-jira`'s shape (FastAPI over
SQLite, thin MCP server, two containers, no auth) before implementation starts.
